...
def _build_node_url(database_url: str, session_name: str, node: str) -> str:
	"""Build URL for /<session>/<node>.json."""
	base = database_url.rstrip("/")
	encoded_session = quote(session_name, safe="")
	encoded_node = quote(node, safe="")
	return f"{base}/{encoded_session}/{encoded_node}.json"


def build_geo_objects_url(database_url: str, session_name: str) -> str:
	"""Build URL for /<session>/geoObjects.json."""
	return _build_node_url(database_url, session_name, GEO_OBJECTS_NODE)


def build_client_requests_url(database_url: str, session_name: str) -> str:
	"""Build URL for /<session>/clientRequests.json."""
	return _build_node_url(database_url, session_name, CLIENT_REQUESTS_NODE)
...
def _fetch_snapshot(url: str) -> dict[str, dict[str, Any]]:
	"""Download and normalize a node snapshot from Firebase."""
	try:
...
def fetch_client_requests(database_url: str, session_name: str) -> dict[str, dict[str, Any]]:
	"""Fetch current /<session>/clientRequests snapshot."""
...
def _iter_stream_from_url(url: str):
	"""Yield parsed StreamEvent objects from a Firebase REST streaming URL."""
...
def iter_firebase_stream(database_url: str, session_name: str):
	...
	yield from _iter_stream_from_url(build_geo_objects_url(database_url, session_name))


def iter_firebase_client_stream(database_url: str, session_name: str):
	"""Yield parsed StreamEvent objects from the clientRequests path."""
	yield from _iter_stream_from_url(build_client_requests_url(database_url, session_name))
