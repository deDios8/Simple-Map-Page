...
export const TRIGGER_COMPONENT_OPTIONS = Object.entries(mapCriteriaComponents)
  ...
  .map(([name, meta]) => ({ value: name, label: meta.short_name || name }));
...
export const RESULT_COMPONENT_OPTIONS = Object.entries(mapResultComponents).map(([name, meta]) => ({
  value: name,
  label: meta.short_name || meta.label || name,
}));
